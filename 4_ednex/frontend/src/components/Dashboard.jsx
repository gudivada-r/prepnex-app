        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
            ...
            >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    ...
                    <div className="stat-card-glass" style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ fontSize: '1.75rem', fontWeight: '700' }}>{userData?.gpa?.toFixed(2) || '0.00'}</div>
                        <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>Current GPA</div>
                    </div>
                    <div className="stat-card-glass" style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ fontSize: '1.75rem', fontWeight: '700' }}>{userData?.on_track_score || '0'}%</div>
                        <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>On-track score</div>
                    </div>
                    {userData?.tuition_balance !== undefined && (
                        <div className="stat-card-glass" style={{ padding: '0.75rem 1rem', background: userData?.has_financial_hold ? 'linear-gradient(135deg, rgba(239,68,68,0.4), rgba(185,28,28,0.4))' : undefined }}>
                            <div style={{ fontSize: '1.75rem', fontWeight: '700' }}>${userData?.tuition_balance?.toFixed(0) || '0'}</div>
                            <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>{userData?.has_financial_hold ? '⚠️ Financial Hold' : 'Tuition Balance'}</div>
                        </div>
                    )}
                </div>
            </motion.div>
    ...
    const fetchUser = async () => {
        ...
        try {
            ...
            try {
                ...
                console.log("EdNex Context Payload:", edNexRes.data);
